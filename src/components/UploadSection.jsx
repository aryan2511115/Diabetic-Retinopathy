...
export default function UploadSection() {
  const { setImage, imagePreview, isAnalyzing, setAnalyzing, setResult, setError } = useStore();
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      setResult(result);
      // Wait for results to be shown and then scroll
      setTimeout(() => {
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div
            ...
            className={`w-full h-[350px] rounded-3xl border-2 border-dashed transition-all duration-300 relative group cursor-pointer flex flex-col items-center justify-center gap-4 overflow-hidden shadow-inner ${
                isDragActive ? "border-medical-primary bg-medical-primary/10" : "border-medical-border hover:border-gray-500 hover:bg-white/5"
            }`}
